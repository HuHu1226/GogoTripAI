...
def build_rag_prompt(rag_retriever: CacheRetriever, street_name, prompt):
    ...
    chunk, db_context, references = real_retriever.query(
        f"商品名：{street_name}。{prompt}", context_max_length=CONTEXT_MAX_LENGTH - 2 * len(GENERATE_TEMPLATE)
    )
...
def gen_rag_db(force_gen=False):
    ...
    if Path(WEB_CONFIGS.street_INSTRUCTION_DIR_GEN_DB_TMP).exists():
        shutil.rmtree(WEB_CONFIGS.street_INSTRUCTION_DIR_GEN_DB_TMP)
    Path(WEB_CONFIGS.street_INSTRUCTION_DIR_GEN_DB_TMP).mkdir(exist_ok=True, parents=True)
    ...
    for _, info in street_info_dict.items():
        shutil.copyfile(
            info["instruction"], Path(WEB_CONFIGS.street_INSTRUCTION_DIR_GEN_DB_TMP).joinpath(Path(info["instruction"]).name)
        )
    ...
    gen_vector_db(
        ...
        str(Path(WEB_CONFIGS.street_INSTRUCTION_DIR_GEN_DB_TMP).absolute()),
