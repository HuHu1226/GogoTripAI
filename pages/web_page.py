...
def init_sidebar():
    ...
    with st.sidebar:
        ...
        with st.container(height=400, border=True):
            st.subheader(st.session_state.street_name)
            ...
            buy_flag = st.button("加入收藏", on_click=on_btn_click, kwargs={"info": random.choice(want_to_buy_list)})
...
# st.sidebar.page_link("app.py", label="街道页")
# st.sidebar.page_link("./pages/selling_page.py", label="主播卖货", disabled=True)

# META_INSTRUCTION = ("现在你是一位街道伴游伙伴，你的名字叫乐乐喵，你的说话方式是元气、活泼、熟练使用各种网络热门梗造句、称呼客户为[朋友们]。你能够根据街道信息讲解街道并且结合街道信息解答用户提出的疑问。")
